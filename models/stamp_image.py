...
class StampImages(Base):
    ...
    create_dt = Column(DateTime, nullable=False, comment="업로드 일자", server_default=func.current_timestamp())
    is_delete = Column(Boolean, default=False, comment="삭제 여부 플래그")
    delete_dt = Column(DateTime, nullable=True, comment="삭제 일자", default=None)
