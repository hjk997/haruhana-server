...
def upload_file(user_id: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        s3_client.upload_fileobj(file.file, BUCKET_NAME, DIRECTORY_NAME + file_id)
...
def download_file(file_id: str):
    ...
    try:
        url = s3_client.generate_presigned_url(
            "get_object",
            Params={"Bucket": BUCKET_NAME, "Key": DIRECTORY_NAME + file_id},
...
def delete_file(object_name: str):
    ...
    try:
        s3_client.delete_object(Bucket=bucket_name, Key=DIRECTORY_NAME + object_name)
        return True
    except Exception as e:
        # 필요시 로깅
